...
- Continues oldest-first across bounded batches, persists the accepted frozen cutoff as a drain entitlement, and drains it exactly once even if ownership moves to another service after a partial commit. A completed old drain does not authorize a newer below-threshold cutoff. When more than three batches remain, work continues in yielded slices, and the configured maximum threshold remains supported.
