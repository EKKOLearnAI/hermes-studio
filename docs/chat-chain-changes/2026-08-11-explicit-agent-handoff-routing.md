...
Group Chat Agent replies now distinguish conversational `@Participant` references from routing intent. The Agent adapter emits authoritative `mentions: []` for replies without a final handoff line, so visible explanatory references remain ordinary persisted text and never fall back to legacy name parsing. A final non-empty line beginning with one or more consecutive `@Participant` tokens remains an explicit handoff; only that leading mention prefix is converted to validated structured participant metadata, while later explanatory references on the same line remain text.
