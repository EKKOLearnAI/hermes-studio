...
The floating voice transcript overlay uses `--bg-input`, `--text-primary`, and `--border-color` instead of Naive popover variables, so light mode renders a light tip and dark mode renders a dark tip.
