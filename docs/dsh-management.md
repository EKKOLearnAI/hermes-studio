...
Each turn initializes ACP, creates or resumes the stored native session, applies the selected model, and sends text/image content. Studio loads a small Cordis plugin from the private runtime home to forward DSH's native `agent/assistant-stream` text and reasoning deltas over a private ACP notification. Only the owned ACP session's live output reaches chat; subagent and auxiliary proxy requests are not forwarded. Committed ACP messages are deduplicated against their live attempt by message ID, while final-only output remains supported. Tools, completion and context updates still come from ACP. This applies to scoped and global runs across single chat, group chat and workflows, without changing the installed DSH package. Scoped billing uses the proxy usage ledger; ACP context occupancy is not counted as billed tokens. Global usage may be estimated. Native `/compact` resumes the same native session in a short-lived ACP process and invokes the selected preset’s compaction backend. DSH chooses the history range, generates the summary, replaces native context and persists it; Studio keeps its displayed transcript and updates context occupancy from the native token meter. No assistant prompt or synthetic summary is added by Studio.

The private ACP copy exposes an optional `_ekko/compact` request for this operation. Compaction code and process ownership stay in `services/dsh`; shared session commands only dispatch and temporarily restore an absent runner. This does not modify the installed DSH package or require a separate DSH server. If a future release changes this optional registration point, ordinary chat adaptation remains available. Presets without a compaction backend report that capability as unavailable. `/compact` accepts no arguments, requires saved native history and rejects active turns or another compaction. Stop cancels native maintenance, and a later attempt can resume the same history. The request has a five-minute deadline; summary and persistence errors are reported without replacing the session. If DSH finds no compactable history, Studio reports completion without changes.
