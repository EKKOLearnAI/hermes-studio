...
| 2026-06-04 | #1314 `5106912` | Aurora shell / chat input draft / tool payload UI rebase cleanup | Rebased the Aurora/code-intelligence branch onto latest `main`, restored chat tool-result unified diff helpers in `MessageItem.vue`, kept per-session composer drafts in `ChatInput.vue`, and kept the gateway bootstrap service import available for server startup. This affects Chat rendering of tool payloads, local composer draft persistence, and gateway startup wiring only; it does not change `/chat-run` event names, message persistence schema, approval semantics, or run lifecycle. |
