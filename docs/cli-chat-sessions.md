...
| --- | --- | --- | --- |
| 2026-06-03 | #1299 `cdeb6b8` | AI session title generation | 普通 Chat 在首轮 assistant reply 落库后可由服务端生成短标题；`run.completed` 携带 `title_generation` 元数据，前端只应用服务端结果。标题生成通过 Agent Bridge `auxiliary_llm` 请求进入 profile worker，multi-profile broker 也会按 profile 转发该 action；不改变消息流式、工具审批、澄清、队列或压缩协议。 |
