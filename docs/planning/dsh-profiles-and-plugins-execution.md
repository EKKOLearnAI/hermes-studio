...
状态：规划 v0.2，尚未实施。作为[主规划](dsh-profiles-and-plugins.md)的实施附件，随 [PR #3011](https://github.com/EKKOLearnAI/hermes-studio/pull/3011)维护。下文 API、事件名、错误码及测试文件名均为拟议契约；源码核对不等于运行测试通过。

## 1. 本轮收敛的技术决定

| ID | 决定 | 依据与状态 |
| --- | --- | --- |
| D01 | 配置来源与启动入口分离；用户可选 Web 来源，Studio 启动自有 ACP 组合 | 设计确定；实际 Web 组合转换待 M0-A 验证 |
| D02 | 一 owner 一进程一根原生会话；不按模型或 Profile 跨 owner 共享进程 | 第一版设计确定；状态保持待 M0-B 验证 |
| D03 | 预设只能在工厂 setup 中 mount，子 Agent composeFrom 父组合 | rc.1 源码已核对；接入适配器仍待验证 |
| D04 | 默认准备版本固定的 Studio ACP 适配插件；官方将来提供等价接口再替换 | 实施路径确定；发布 API、维护范围、许可证及兼容性是 M0-A 门槛 |
| D05 | 初版不自动继承 Web 浏览器执行能力，不自动导入官方会话历史 | 范围确定；后端兼容性不能替代浏览器验收 |
| D06 | 包修改产生新 revision，运行中的实例继续持有旧 revision | 设计确定；不得原地修改活动依赖 |
| D07 | 后端完成以输出排空、持久化屏障及 Studio 记录完成为准，不以子进程退出为准 | 设计确定；非销毁式 flush 待 M0-B 验证 |
...
所有任务状态均为“待实施”。主要落点表示代码职责，不表示已经分配了子 Agent 或开发人员。
...
每项在实施时附测试命令、DSH/平台版本、fixture revision、断言与日志位置。当前没有将下列场景标为通过。
...
下一次进入实现时先完成 T01 和 T02：产出实际可复现的预设绑定与持续会话实验，再开发来源选择页面。当前已有的 `dsh-acp-real.test.ts` 是单轮进程/跨进程恢复基线，不能直接当成新的同进程状态保留证据。

建议新增证据文件 `docs/planning/dsh-compatibility-evidence.md`，仅在真实实验完成时创建，记录 API 可用性、组合差异、flush 语义和浏览器方案结论。证据不通过就保持对应任务待完成，不把这份规划自身作为验收成果。
