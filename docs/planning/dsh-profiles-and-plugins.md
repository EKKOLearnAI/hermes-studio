...
预设绑定优先使用 DSH 公开的生命周期和组合接口，验证在工具目录、提示词、Skills 读取前完成加入。需要覆盖新建、恢复、子 Agent 继承和两个不同预设并存。

如果公开 hook 无法正确介入 ACP Agent 创建，进入明确的技术决策：向上游补可选预设接口，或维护 Studio 自有的 ACP 适配插件。不得通过直接修改全局 npm 安装内容解决，也不使用时序猜测式 monkey patch。决策及所支持的 DSH 版本必须记录后，才能宣称 Web 预设可用。
...
| 问题 | 所需证据 | 决定点 |
| --- | --- | --- |
| ACP 创建/恢复如何绑定预设 | 官方 hook / 上游接口的真实组合实验 | M0 结束前；不通过则记录上游依赖或自有适配方案 |
