...
> 说明：当前版本尚未提供应用内“自定义字体 / 字号”开关；如果默认字体过大、过小或长时间阅读导致眼睛疲劳，可先使用下列系统和浏览器级方案。该说明用于跟踪 [#1835](https://github.com/EKKOLearnAI/hermes-studio/issues/1835) 中的字体舒适度诉求。

- 桌面应用用户可以优先使用系统显示缩放：Windows 的“设置 → 系统 → 显示 → 缩放”，macOS 的“系统设置 → 显示器 → 缩放”，Linux 桌面环境的 Display / Scale 选项。
- 浏览器访问 Web UI 时，可使用 `Ctrl`/`Cmd` + `+` 放大、`Ctrl`/`Cmd` + `-` 缩小、`Ctrl`/`Cmd` + `0` 恢复默认缩放。
- 如果只觉得聊天内容拥挤，可在 Settings → Display 中开启或关闭紧凑模式，观察消息间距、工具调用轨迹和侧边栏密度是否更适合当前屏幕。
- 如果推理过程或费用信息造成阅读负担，可在 Settings → Display 中隐藏对应显示项，减少同屏文字量。
- 对高分辨率屏幕，建议先尝试 110%–125% 浏览器缩放；对小屏笔记本，建议先尝试系统 125%–150% 缩放，再按需微调浏览器缩放。
- 对外接显示器，建议分别记录每台显示器的分辨率、缩放比例和 Hermes Studio 窗口大小，因为同一字号在不同 DPI 下体感差异很大。
- 如果字体显得发虚或边缘闪烁，请确认操作系统的字体平滑、ClearType（Windows）或等效抗锯齿设置已启用。
- 如果中文、英文或代码块的可读性差异明显，请在反馈中说明具体区域：侧边栏、聊天气泡、输入框、终端、代码块、设置页或日志视图。
- 如果只有终端文字不舒适，优先调整浏览器 / 系统缩放；若仍不合适，请单独反馈终端字体大小、行高和当前 shell 输出截图。
- 如果只有代码块或 Markdown 表格过宽，优先调整窗口宽度或浏览器缩放，并反馈是否出现横向滚动、换行过早或内容被截断。
- 长时间使用时建议开启系统夜间模式或降低蓝光，并配合系统级高对比度 / 深色模式；这些设置通常比单独改应用字号更稳定。
- 在多人共享机器或投屏场景中，建议使用浏览器缩放而不是系统缩放，避免影响其它应用的布局。
- 如果放大后侧边栏、弹窗或按钮被挤压，请记录缩放比例和窗口尺寸，这类信息可帮助判断是否需要响应式布局修复。
- 如果缩小后文字仍过大，请同时检查浏览器默认字体大小、页面缩放、系统缩放和显示器缩放是否叠加。
- 提交自定义字体 / 字号请求时，请包含平台、Hermes Studio 版本、安装方式（桌面 / npm / Docker）、浏览器名称和版本。
- 请附上当前系统缩放比例、浏览器缩放比例、显示器分辨率、是否启用紧凑模式，以及最不舒服的页面区域。
- 为便于设计默认值，请说明期望的字体族（例如系统默认、等宽字体、思源黑体等）、目标字号范围和是否需要分别配置聊天区、输入框、终端。
- 若涉及无障碍需求，请说明是否依赖屏幕阅读器、高对比度主题、系统大字模式或键盘导航，以避免后续实现破坏辅助技术体验。
- 截图反馈请尽量同时包含完整窗口和局部放大图；如果包含隐私内容，请先遮挡会话、Token、用户名和文件路径。
- 对开发者而言，字体相关变更应优先保证布局不溢出、焦点态可见、按钮可点击区域不变小，并在中英文混排和代码块场景中验证。
- 在正式提供应用内字体设置前，上述做法是推荐的临时缓解路径；后续如果实现自定义字体 / 字号，应在 Settings → Display 中集中暴露并持久化到用户偏好。
