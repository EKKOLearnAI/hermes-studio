...
export function dshModuleViolations(filename, source) {
  ...
  if (file.startsWith('packages/client/src/')) {
    ...
    const leaksDsh = source.includes('settings.plugin.item') || source.includes('STUDIO_DSH_UI_READY') || source.includes('/api/coding-agents/dsh/ui-session')
  ...
  const failures = []
  for (const token of ['DSH_PERMISSION_MODE', 'studio-web-acp', '@deepseek-ai/dsh-web-app', 'writeDshAcpAdapter', 'prepareDshWebProfile', 'new DshAcpTurn(', 'STUDIO_DSH_UI_READY', 'prepareDshManagementProfile', 'DSH_UI_SLOT_CLIENT']) {
