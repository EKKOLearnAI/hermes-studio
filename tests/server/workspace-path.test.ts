...
describe('workspace path validation', () => {
  ...
  it('resolves valid folders under the configured base', async () => {
    ...
    await mkdir(folder)
    const { resolveAllowedWorkspaceFolder } = await import('../../packages/server/src/modules/hermes/services/files/workspace-path')
  ...
  it('rejects traversal outside the base', async () => {
    ...
    const { resolveAllowedWorkspaceFolder, assertAllowedWorkspaceFolder } = await import('../../packages/server/src/modules/hermes/services/files/workspace-path')
  ...
  it('normalizes native Windows drive paths when drive mode is active', async () => {
    ...
    try {
      const { normalizeWindowsWorkspacePath } = await import('../../packages/server/src/modules/hermes/services/files/workspace-path')
  ...
  it('requires realpath containment on Windows when WORKSPACE_BASE is configured', async () => {
    ...
    Object.defineProperty(process, 'platform', { value: 'win32' })
    try {
      const { isWorkspaceListPathAllowed } = await import('../../packages/server/src/modules/hermes/services/files/workspace-path')
