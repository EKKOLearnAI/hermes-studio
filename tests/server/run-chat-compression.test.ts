...
describe('run chat compression trigger', () => {
  ...
  it('does not rename the session when LLM compression returns a generated title', async () => {
    const messages = Array.from({ length: 10 }, (_, index) => ({
      id: index + 1,
      session_id: 'session-1',
      role: index === 9 ? 'user' : index % 2 === 0 ? 'user' : 'assistant',
    ...
    compressorCompressMock.mockResolvedValue({
      ...
      meta: {
        ...
        verbatimCount: 0,
        compressedStartIndex: 0,
        generatedTitle: 'Compaction title',
      },
    })
    const emit = vi.fn()

    const { buildCompressedHistory } = await import('../../packages/server/src/services/hermes/run-chat/compression')
    await buildCompressedHistory('session-1', 'default', 'http://upstream', undefined, emit, new Map())

    expect(updateAutoGeneratedSessionTitleMock).not.toHaveBeenCalled()
    expect(emit).not.toHaveBeenCalledWith('session.title.updated', expect.anything())
  })

  it('does not request or apply compaction titles when compression.auto_title is false', async () => {
    const messages = Array.from({ length: 10 }, (_, index) => ({
      id: index + 1,
      session_id: 'session-1',
      role: index === 9 ? 'user' : index % 2 === 0 ? 'user' : 'assistant',
    ...
    getSessionDetailMock.mockReturnValue({ messages })
    readConfigYamlForProfileMock.mockResolvedValue({ compression: { auto_title: false } })
    calcAndUpdateUsageMock.mockResolvedValue({ inputTokens: 160_000, outputTokens: 0 })
    compressorCompressMock.mockResolvedValue({
      messages: [{ role: 'user', content: 'compressed' }],
      meta: {
    ...
    const { buildCompressedHistory } = await import('../../packages/server/src/services/hermes/run-chat/compression')
    await buildCompressedHistory('session-1', 'default', 'http://upstream', undefined, vi.fn(), new Map())

    expect(compressorConstructorMock).toHaveBeenCalledWith(expect.objectContaining({
      config: expect.objectContaining({ autoTitle: false }),
    }))
    expect(updateAutoGeneratedSessionTitleMock).not.toHaveBeenCalled()
  })
