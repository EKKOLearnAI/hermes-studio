...
describe('hermes-web-ui MCP server', () => {
  ...
  it('advertises bounded health-loop discovery without weakening HTTP authorization or exposing secrets', () => {
    ...
    expect(source).toContain('hermes_studio_api_openapi_get')
    const openapi = JSON.parse(readFileSync('docs/openapi.json', 'utf8'))
    expect(openapi.tags).toContainEqual(expect.objectContaining({
      name: 'Health Loop',
      description: expect.stringContaining('Protected health'),
    }))
    expect(Object.keys(openapi.paths)).toContain('/api/hermes/health-loop/connectors/{id}/sync')
