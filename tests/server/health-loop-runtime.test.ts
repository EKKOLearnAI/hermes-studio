...
describe('health-loop runtime', () => {
  ...
  it('refreshes exact controller targets and production analysis dependencies across settings and artifacts', async () => {
    ...
    try {
      ...
      const intervening = context({ idempotencyKey: 'sync-controller-2' }, 'health-state')
      await controller.syncConnector(intervening)
      expect(intervening.body.policyDecision.outcome).toBe('allow')
