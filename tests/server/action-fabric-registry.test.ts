...
describe('action fabric registry', () => {
  ...
  it('preserves the Phase 3 capability contract digests while extending the built-in registry', () => {
    ...
    expect(listFabricExecutors().filter(item => !item.id.startsWith('health-') && !item.id.startsWith('home-')).map(item => [item.id, item.type, item.environment, item.configuration.externalWrite])).toEqual([
  ...
  it('reseed is idempotent and never overwrites operator edits or runtime state', () => {
    ...
    expect(withActionFabricDb(db => db.prepare("SELECT COUNT(*) AS count FROM fabric_executor_capabilities WHERE capability_id NOT LIKE 'health.%' AND capability_id NOT LIKE 'home.%'").get()))
      .toEqual({ count: 3 })
  })
