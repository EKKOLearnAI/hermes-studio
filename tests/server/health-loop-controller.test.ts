...
const broker=vi.hoisted(()=>({issue:vi.fn(),revoke:vi.fn(),reserve:vi.fn(),reserveIdempotent:vi.fn()}))
...
describe('health loop controller',()=>{
  beforeEach(()=>{vi.clearAllMocks();settings.getHealthAutomationSettings.mockReturnValue({...settingsValue});settings.updateHealthAutomationSettings.mockReturnValue({...settingsValue,version:2,actorUserId:'42'});
    ...
    vault.store.mockResolvedValue({id:ARTIFACT,mediaType:'application/json',sizeBytes:2,relativePath:'private/path',contentHash:'a'.repeat(64),metadata:{healthAnalysis:{purpose:'diet'},token:'hidden',notes:['Bearer abc.def_ghi-123','C:\\Users\\alice\\health.db','/home/alice/private/x.db','safe note']},source:'health-loop-api',sourceId:'upload-1',sensitivity:'health',createdAt:'2026-07-14T00:00:00.000Z'})
    ...
    broker.reserveIdempotent.mockResolvedValue({reservationId:'reservation-stable',consentId:'consent-1',reservedAt:'2026-07-14T00:00:00.000Z',expiresAt:'2026-07-14T00:05:00.000Z'})
  ...
  it('stores a bounded multipart artifact and recursively sanitizes sensitive metadata keys and values',async()=>{const ctrl=await import('../../packages/server/src/controllers/hermes/health-loop');const input=multipart();const c=ctx();c.get=vi.fn((name:string)=>name==='content-type'?input.contentType:String(input.body.length));c.req=Readable.from([input.body]);await ctrl.createArtifact(c);expect(c.status).toBe(201);expect(c.body.artifact).toMatchObject({id:ARTIFACT,manifestDigest:DIGEST});expect(JSON.stringify(c.body)).not.toMatch(/private\/path|hidden|token|Bearer|Users|health\.db|\/home\/alice/i);expect(c.body.artifact.metadata.notes).toEqual(['[redacted]','[redacted]','[redacted]','safe note'])})

  it.each([{notes:[{text:'Bearer hidden'}]},{healthAnalysis:{purpose:123}},{healthAnalysis:{purpose:'medical'}}])('rejects malformed public artifact metadata without echoing it: %j',async metadata=>{const ctrl=await import('../../packages/server/src/controllers/hermes/health-loop');vault.store.mockResolvedValueOnce({id:ARTIFACT,mediaType:'application/json',sizeBytes:2,relativePath:'C:\\private\\health.db',contentHash:'a'.repeat(64),metadata,source:'health-loop-api',sourceId:'upload-1',sensitivity:'health',createdAt:'2026-07-14T00:00:00.000Z'});const input=multipart();const c=ctx();c.get=vi.fn((name:string)=>name==='content-type'?input.contentType:String(input.body.length));c.req=Readable.from([input.body]);await ctrl.createArtifact(c);expect(c.status).toBe(400);expect(c.body).toEqual({error:'Invalid health loop request',code:'HEALTH_REQUEST_INVALID'});expect(JSON.stringify(c.body)).not.toMatch(/Bearer|private|health\.db|medical/i)})
  ...
  it('issues a one-time token only on consent creation, reserves it for remote analysis, and never echoes it afterward',async()=>{const ctrl=await import('../../packages/server/src/controllers/hermes/health-loop');const manifest={artifactIds:[ARTIFACT],processor:'vision',purpose:'diet',selectedRegions:[],requestedFields:['caloriesKcal'],retention:'session'};const create=ctx({manifest});await ctrl.createConsent(create);expect(create.body.consent.token).toBe(TOKEN)
    const analyze=ctx({mode:'remote',manifestDigest:DIGEST,processorId:'vision',consentToken:TOKEN,manifest,idempotencyKey:'analysis-remote-1'},{id:ARTIFACT});await ctrl.analyzeArtifact(analyze);expect(broker.reserveIdempotent).toHaveBeenCalledWith(TOKEN,manifest,expect.objectContaining({artifactId:ARTIFACT}),{actorUserId:'42',idempotencyKey:'analysis-remote-1'});expect(JSON.stringify(analyze.body)).not.toContain(TOKEN)
  ...
  it('reuses the same durable consent reservation and Fabric key after a post-reserve failure',async()=>{const ctrl=await import('../../packages/server/src/controllers/hermes/health-loop');const manifest={artifactIds:[ARTIFACT],processor:'vision',purpose:'diet',selectedRegions:[],requestedFields:['caloriesKcal'],retention:'session'};const body={mode:'remote',manifestDigest:DIGEST,processorId:'vision',consentToken:TOKEN,manifest,idempotencyKey:'analysis-loss-1'};fabric.createFabricIntent.mockImplementationOnce(()=>{throw new Error('provider response lost')});const first=ctx(body,{id:ARTIFACT});await ctrl.analyzeArtifact(first);expect(first.status).toBe(503);const retry=ctx(body,{id:ARTIFACT});await ctrl.analyzeArtifact(retry);expect(retry.status).toBe(202);expect(broker.reserveIdempotent).toHaveBeenCalledTimes(2);expect(fabric.createFabricIntent).toHaveBeenLastCalledWith(expect.objectContaining({idempotencyKey:'analysis-loss-1',input:expect.objectContaining({consentId:'reservation-stable',requestedAt:'2026-07-14T00:00:00.000Z'})}));expect(JSON.stringify([first.body,retry.body])).not.toContain(TOKEN)})
})
