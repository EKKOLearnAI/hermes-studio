...
describe('LAN discovery', () => {
  ...
  it('exposes an MCP terminal list tool so agents can recover forgotten terminal ids', () => {
    ...
    expect(mcpSource).toContain('token: managed ? undefined : args.token')
    expect(mcpSource).toContain('profile: managed?.profile || args.profile')
    expect(mcpSource).toContain('Managed MCP authentication is fixed by the signed capability')
