...
import { createAppImagePreview } from '../../packages/server/src/modules/hermes/services/files/app-image-preview'
...
describe('createAppImagePreview', () => {
  ...
  it('does not load Sharp for non-image downloads', async () => {
    ...
    const { createAppImagePreview: createWithoutSharp } = await import(
      '../../packages/server/src/modules/hermes/services/files/app-image-preview'
    )
    const source = Buffer.from('hello')
