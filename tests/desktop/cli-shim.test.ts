...
describe('Hermes Studio CLI shim', () => {
  ...
  it('routes Windows cli and web subcommands through bundled runtime paths', () => {
    ...
    expect(content).toContain('set "PYTHON=%RUNTIME%\\python\\venv\\python.exe"')
    expect(content).toContain('if not exist "%PYTHON%" set "PYTHON=%RUNTIME%\\python\\python.exe"')
    expect(content).toContain('set "VIRTUAL_ENV=%RUNTIME%\\python\\venv"')
    expect(content).toContain('if not exist "%VIRTUAL_ENV%\\python.exe" set "VIRTUAL_ENV=%RUNTIME%\\python"')
    expect(content).toContain('set "UV_PROJECT_ENVIRONMENT=%VIRTUAL_ENV%"')
    expect(content).toContain('set "UV_PYTHON=%PYTHON%"')
    expect(content).toContain('set "UV_SYSTEM_PYTHON=1"')
