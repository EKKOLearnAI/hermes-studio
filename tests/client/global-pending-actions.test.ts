...
describe('GlobalPendingActions', () => {
  ...
  it('renders the exact approval command as a scrollable code block and copies it', async () => {
    ...
    expect(content.get('.global-approval-agent').text()).toBe('chat.approvalAgentHermes Agent')
    expect(content.findAll('.global-approval-agent')[1].text()).toBe('chat.approvalSummary')
    expect(content.get('.global-approval-description').text()).toBe('chat.approvalRiskSecurity scan')
