...
describe('GroupChatPanel workspace save handling', () => {
  ...
  it('renders the persistent room Agent grid in the stable leading room-list slot', () => {
    ...
    expect(roomAvatar).toMatch(/&\.is-active::before\s*\{[^}]*inset: -1px;[^}]*border: 1px solid #fff;[^}]*box-shadow: 0 0 0 2px rgba\(var\(--accent-primary-rgb\), 0\.9\);/s)
    expect(roomAvatar).toMatch(/&\.is-active::after\s*\{[^}]*inset: -5px;[^}]*animation: room-agent-grid-breathe 1\.6s ease-in-out infinite;/s)
    expect(roomAvatar).toContain('@keyframes room-agent-grid-breathe')
    expect(roomAvatar).not.toContain('@keyframes room-agent-grid-pulse')
    ...
    expect(runCard).toMatch(/\.run-avatar-active::before,\s*\.run-avatar-active::after\s*\{[^}]*border-radius: 50%;/s)
    expect(runCard).toMatch(/\.run-avatar-active::before\s*\{[^}]*border: 1px solid #fff;[^}]*box-shadow: 0 0 0 2px rgba\(var\(--accent-primary-rgb\), 0\.9\);/s)
    expect(runCard).toMatch(/\.run-avatar-active::after\s*\{[^}]*animation: run-avatar-active-glow 1\.6s ease-in-out infinite;/s)
