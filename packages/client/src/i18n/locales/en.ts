export default {
  ...
  groupChat: {
    ...
    fullLocalAccessHint: 'When enabled, local group-chat Agents triggered by non-owner members are no longer scoped to the room workspace and can read/write every drive and config path on this machine. Applies to this room only; owner-only setting.',
    fullLocalAccessDangerHint: 'Risk: once enabled, any member who can trigger an Agent in this room will be able to drive it to read/write local files, including configuration and credential paths.',
    fullLocalAccessConfirmTitle: 'Enable full local access?',
    fullLocalAccessConfirmContent: 'Non-owner-triggered local Agents will get the same file access as the machine owner (all drives and config paths). Applies to this room only; the workspace restriction is restored immediately when disabled.',
    fullLocalAccessSaved: 'Full local access setting saved',
    fullLocalAccessSaveFailed: 'Failed to save full local access setting',
