export default {
  ...
  groupChat: {
    ...
    fullLocalAccess: '전체 로컬 액세스',
    fullLocalAccessHint: '활성화하면 비소유자 멤버가 트리거한 로컬 그룹 채팅 Agent가 작업 공간 디렉터리 제한을 받지 않고 이 머신의 모든 드라이브와 설정 경로를 읽고 쓸 수 있습니다. 이 방에만 적용되며 소유자만 수정할 수 있습니다.',
    fullLocalAccessDangerHint: '위험: 활성화하면 이 방에서 Agent를 트리거할 수 있는 모든 멤버가 설정 및 자격 증명 경로를 포함한 로컬 파일을 읽고 쓸 수 있게 됩니다.',
    fullLocalAccessConfirmTitle: '전체 로컬 액세스를 활성화하시겠습니까?',
    fullLocalAccessConfirmContent: '활성화하면 비소유자 멤버가 트리거한 로컬 Agent가 머신 소유자와 동일한 파일 액세스 권한(모든 드라이브 및 설정 경로)을 얻습니다. 이 방에만 적용되며 비활성화하면 즉시 작업 공간 제한이 복원됩니다.',
    fullLocalAccessSaved: '전체 로컬 액세스 설정이 저장되었습니다',
    fullLocalAccessSaveFailed: '전체 로컬 액세스 설정 저장 실패',
