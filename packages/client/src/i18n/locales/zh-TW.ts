export default {
  ...
  groupChat: {
    ...
    fullLocalAccess: '本機完全存取',
    fullLocalAccessHint: '啟用後，非房主訊息觸發的本機群聊 Agent 不再受限於工作區目錄，可讀寫本機所有磁碟與設定路徑。僅對本房間生效，僅房主可修改。',
    fullLocalAccessDangerHint: '風險提示：啟用後，本房間內任何能觸發 Agent 的成員都能驅動其讀寫本機檔案（包括設定與憑證路徑）。',
    fullLocalAccessConfirmTitle: '確認啟用「本機完全存取」？',
    fullLocalAccessConfirmContent: '啟用後，非房主成員觸發的本機 Agent 將獲得與本機擁有者相同的檔案存取權限（所有磁碟與設定路徑）。此設定僅對本房間生效，停用後立即恢復工作區限制。',
    fullLocalAccessSaved: '本機完全存取設定已儲存',
    fullLocalAccessSaveFailed: '本機完全存取設定儲存失敗',
