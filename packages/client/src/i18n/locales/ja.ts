export default {
  ...
  runtimeVersions: {
    ...
    runtimeDirectoryHint: '次回再起動時にローカルサービスの起動前に現在の Runtime をコピーして検証します。元のコピーは保持されます。',
    runtimeDirectorySaved: 'Runtime の移行を予約しました。Hermes Studio を再起動すると実行されます。',
    runtimeMigrationPending: '移行先：{directory}',
    runtimeMigrationFailed: 'Runtime の移行に失敗したため、以前の Runtime を引き続き使用します',
    active: '有効',
    installed: 'インストール済み',
    download: 'ダウンロード',
  ...
  changelog: {
    ...
    new_0_6_31_12: 'バージョン管理で Runtime の保存先を移行できるようになりました。次回再起動時にサービス開始前にコピーと検証を行い、元のコピーも保持します（#2134）',
