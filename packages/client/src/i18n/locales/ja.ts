export default {
  ...
  groupChat: {
    ...
    fullLocalAccess: 'フルローカルアクセス',
    fullLocalAccessHint: '有効にすると、非オーナーメンバーがトリガーしたローカル群聊 Agent はワークスペースディレクトリの制限を受けず、このマシンのすべてのドライブと設定パスを読み書きできます。この部屋のみに適用され、オーナーのみが変更できます。',
    fullLocalAccessDangerHint: 'リスク: 有効にすると、この部屋で Agent をトリガーできるメンバーは、設定や資格情報のパスを含むローカルファイルを読み書きできるようになります。',
    fullLocalAccessConfirmTitle: 'フルローカルアクセスを有効にしますか？',
    fullLocalAccessConfirmContent: '有効にすると、非オーナーメンバーがトリガーしたローカル Agent はマシン所有者と同じファイルアクセス権（すべてのドライブと設定パス）を取得します。この部屋のみに適用され、無効にするとすぐにワークスペース制限が復元されます。',
    fullLocalAccessSaved: 'フルローカルアクセス設定を保存しました',
    fullLocalAccessSaveFailed: 'フルローカルアクセス設定の保存に失敗しました',
