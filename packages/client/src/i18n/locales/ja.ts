export default {
  ...
  sidebar: {
    ...
    connected: '接続済み',
    disconnected: '未接続',
    updateTip: 'ターミナルで "audbuild update" を実行して更新してください',
  ...
  chat: {
    ...
    emptyState: 'audbuild と会話を開始しましょう',
    ...
    openHistory: '履歴を開く',
    hermesHistory: 'audbuild 履歴',
    historyScopeHint: 'ソース別にグループ化された audbuild 履歴セッションを読み取り専用で表示します。',
  ...
  plugins: {
    title: 'プラグイン',
    refresh: '更新',
    notice: '検出可能な audbuild プラグイン manifest の読み取り専用インベントリです。検出メタデータはプラグインコードを読み込まずに取得します。v1 の管理操作は CLI のままで、変更は新しい audbuild セッションで有効になります。',
  ...
  models: {
    ...
    copilotAddDetected: 'このマシンで GitHub Copilot OAuth トークンを検出しました。「追加」をクリックして audbuild で Copilot を有効化します。',
    ...
    copilotDeleteHintGhCli: 'Copilot は audbuild 上で非表示になります。gh CLI のログインには影響しません — `gh auth status` は引き続きログイン状態を表示します。',
    copilotDeleteHintAppsJson: 'Copilot は audbuild 上で非表示になります。VS Code Copilot 拡張機能のログインには影響しません。',
    customBadge: 'カスタム',
    previewBadge: 'プレビュー',
    disabledBadge: '利用不可',
  ...
  skillsUsage: {
    title: 'スキル使用状況',
    subtitle: 'audbuild セッションでのスキル読み込みと編集を追跡します',
    refresh: '更新',
  ...
  changelog: {
    ...
    new_0_5_15_9: 'audbuild Markdownメディアレンダリングと同期リトライを修正',
    new_0_5_15_10: 'アップストリーム環境変数依存をリファクタリングで削除',
    new_0_5_15_11: 'カンバン機能が使用できない場合は、audbuild をアップグレードしてください',
    ...
    new_0_5_17_9: '読み取り専用のaudbuildプラグインページを追加',
    ...
    new_0_5_5_2: 'audbuildセッション履歴ページを追加',
