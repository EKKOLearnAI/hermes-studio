export default {
  ...
  models: {
    ...
    copilotLoginIntro: 'Hermes Web UI はローカル gh CLI を介して GitHub 認証を共有します。一度ログインすると認証情報が再利用されます。',
    copilotStep1: 'このマシンに GitHub CLI（gh）がインストールされていることを確認してください。',
    copilotStep2: 'システムターミナルで以下のコマンドを実行してください：',
    copilotStep3: 'ブラウザでの認証完了後、下の「検出を更新」をクリックしてください。',
    copilotLoginNote: '認証はこのマシンの gh CLI によって永続化され、Web UI 再起動後も有効です。',
    copilotCopied: 'コマンドをコピーしました',
    copilotDetected: 'GitHub Copilot を検出しました。利用できます。',
    copilotNotDetected: 'Copilot 認証はまだ検出されていません。`gh auth login` の完了を確認してください。',
    copilotRefresh: '検出を更新',
    customBadge: 'カスタム',
    previewBadge: 'プレビュー',
    disabledBadge: '利用不可',
