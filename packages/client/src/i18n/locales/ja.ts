...
export default {
  ...
  agentManager: {
    ...
    cursorNoManagedConfig: 'Cursor の MCP サーバーは ~/.cursor/mcp.json にあります。MCP パネルで編集してください。CLI に --mcp-config がないため、Studio は管理対象サーバーをそのファイルにマージします。',
  ...
  connections: {
    ...
    app: {
      ...
      pushForDevice: "{name} へのプッシュ通知",
      pushUpdateFailed: "プッシュ通知設定を更新できませんでした",
      online: 'オンライン',
      offline: 'オフライン',
      active: '有効',
      expired: '期限切れ',
  ...
  changelog: {
    new_0_7_23_1: 'App リレー経由の永続的なモバイルターミナルセッションを追加し、出力配信と入力フロー制御を改善 (#3076, #3079)',
    new_0_7_23_2: 'Coding Agent の確認質問と保存可能なグループチャットのタスクカードを追加し、タスク計画を各ターンの末尾に固定 (#3080, #3085)',
    new_0_7_23_3: 'セッションのピン留めをデバイス間で同期し、最近のセッションより上に表示。App のページ分割時のピン留めフィルターも修正 (#3091)',
    new_0_7_23_4: 'App 通知のグループ活動と確認質問の状態復元を改善し、実行スナップショットの永続化、タスク進捗通知イベント、Webhook 購読オプションを追加 (#3090, #3093)',
    new_0_7_23_5: 'ekko-studio npm パッケージと CLI を追加し、hermes-web-ui のアップグレード互換性を維持。デスクトップパッケージのブランドを統一 (#3084)',
    new_0_7_23_6: 'Coding Agent がメッセージ履歴を再生する際に DeepSeek の推論内容が失われる問題を修正 (#3078)',
    new_0_7_23_7: 'MCP サーバーが HTTP トランスポートの別名を使用すると Ekko ランタイムでツールが読み込まれない問題を修正 (#3081)',
    new_0_7_23_8: 'スキルの空のメタデータ値が次のフィールドを読み込む問題を修正し、複数行の説明と空行を維持 (#3083)',
