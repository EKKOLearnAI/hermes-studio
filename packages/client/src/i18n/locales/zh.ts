export default {
  ...
  plugins: {
    title: '插件',
    refresh: '刷新',
    notice: '这里会扫描可发现的 Hermes 插件 manifest，扫描元数据时不会加载插件代码。用户插件现在可以直接安装、启用、禁用、更新或移除；变更会在新的 Hermes 会话生效。',
    loadFailed: '加载插件失败',
    commandCopied: '命令已复制',
    ...
    installEnable: '安装后启用',
    installForce: '覆盖现有插件',
    installMissing: '请输入插件 Git URL 或 owner/repo。',
    installSucceeded: '插件已安装：{name}',
    enableAction: '启用',
    disableAction: '禁用',
    updateAction: '更新',
    removeAction: '移除',
    removeConfirm: '要从用户插件移除“{name}”吗？',
    actionSucceeded: '插件动作已完成：{name}',
    operationFailed: '插件操作失败',
    changesNextSession: '变更会在新的 Hermes 会话生效。',
    missingEnv: '缺少环境变量：{names}',
