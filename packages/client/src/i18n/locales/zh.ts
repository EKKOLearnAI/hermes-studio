export default {
  ...
  sidebar: {
    ...
    collapse: '收起菜单',
    expand: '展开菜单',
    updateTip: '在终端运行 "audbuild update" 即可更新',
  ...
  chat: {
    ...
    openHistory: '打开历史',
    hermesHistory: 'audbuild 历史',
    historyScopeHint: '这里按来源只读查看 audbuild 历史会话。',
    noSessions: '暂无会话',
    searchTitle: '搜索会话',
  ...
  plugins: {
    title: '插件',
    refresh: '刷新',
    notice: '只读展示可发现的 audbuild 插件 manifest。发现元数据读取不会加载插件代码。v1 管理动作仍保留在 CLI，新 audbuild 会话生效。',
    loadFailed: '加载插件失败',
    commandCopied: '命令已复制',
  ...
  models: {
    ...
    copilotAddDetected: '已在本机检测到 GitHub Copilot OAuth 凭证，点击「添加」即可在 audbuild 中启用 Copilot。',
    ...
    copilotDeleteHintGhCli: 'Copilot 将从 audbuild 列表移除。不会影响 gh CLI —— `gh auth status` 仍显示已登录。',
    copilotDeleteHintAppsJson: 'Copilot 将从 audbuild 列表移除。不会影响 VS Code Copilot 插件的登录。',
    customBadge: '自定义',
    previewBadge: '预览',
    disabledBadge: '不可用',
    ...
    aliasHint: '仅修改 Web UI 显示名，发送给 audbuild 的仍是原始模型 ID。',
    ...
    visibilityHint: '仅影响 Web UI 的模型选择器和模型页展示，不会改写 audbuild CLI 的 provider/model 配置。实际调用仍使用原始模型 ID。',
  ...
  skillsUsage: {
    title: '技能用量',
    subtitle: '跟踪 audbuild 会话中的技能加载和编辑',
    refresh: '刷新',
  ...
  changelog: {
    ...
    new_0_5_15_9: '修复 audbuild Markdown 媒体渲染与同步重试',
    new_0_5_15_10: '重构移除上游环境变量依赖',
    new_0_5_15_11: '如果看板功能无法使用，请升级 audbuild',
    ...
    new_0_5_17_9: '新增只读 audbuild 插件页',
    ...
    new_0_5_22_1: '修复 audbuild 会话列表接口错误读取 Web UI 本地会话库的问题',
    ...
    new_0_5_5_2: '新增历史页面，用于浏览 audbuild 会话历史记录',
    ...
    new_0_5_4_3: '修复 Docker 部署后 Web 终端无法使用 audbuild CLI 的问题',
    new_0_5_4_4: '添加工作区对话框标题 i18n 翻译，改进会话持久化',
    new_0_5_4_5: '支持代码块复制反馈，显示用户通知',
    new_0_5_4_6: '对齐使用分析与 audbuild 状态数据库架构',
    ...
    new_0_4_8_10: '上传目录从临时目录迁移到 ~/.audbuild/upload',
