import { startRun, streamRunEvents } from '@/api/hermes/chat';
import { deleteSession as deleteSessionApi, fetchSession, fetchSessions } from '@/api/hermes/sessions';
...
function mapHermesMessages(msgs) {
    ...
    for (const msg of msgs) {
        ...
        if (msg.role === 'tool') {
            ...
            const placeholderIdx = result.findIndex(m => m.role === 'tool' && m.toolName === toolName && !m.toolResult && m.id.includes('_' + tcId));
            if (placeholderIdx !== -1) {
...
function storageKey() { return STORAGE_KEY_PREFIX + getProfileName(); }
function sessionsCacheKey() { return SESSIONS_CACHE_KEY_PREFIX + getProfileName(); }
function msgsCacheKey(sid) { return `hermes_session_msgs_v1_${getProfileName()}_${sid}_`; }
function inFlightKey(sid) { return `hermes_in_flight_v1_${getProfileName()}_${sid}`; }
...
function sanitizeForCache(msgs) {
    return msgs.map(m => {
...
export const useChatStore = defineStore('chat', () => {
    ...
    const streamStates = ref(new Map());
    ...
    function persistSessionsList() {
        ...
        saveJson(sessionsCacheKey(), sessions.value.map(s => ({ ...s, messages: [] })));
    }
    function persistActiveMessages() {
        ...
        if (s)
            saveJson(msgsCacheKey(sid), sanitizeForCache(s.messages));
    }
    function markInFlight(sid, runId) {
        saveJson(inFlightKey(sid), { runId, startedAt: Date.now() });
    }
    function clearInFlight(sid) {
        removeItem(inFlightKey(sid));
    }
    function readInFlight(sid) {
        const rec = loadJson(inFlightKey(sid));
        if (!rec)
            return null;
    ...
    async function loadSessions() {
        ...
        try {
            ...
            const cachedSessions = loadJson(sessionsCacheKey());
            if (cachedSessions?.length) {
                ...
                if (savedId) {
                    ...
                    if (cachedActive) {
                        const cachedMsgs = loadJson(msgsCacheKey(savedId));
                        if (cachedMsgs)
                            cachedActive.messages = cachedMsgs;
    ...
    async function switchSession(sessionId) {
        ...
        if (!hasLocalMessages) {
            const cachedMsgs = loadJson(msgsCacheKey(sessionId));
    ...
    async function switchSessionModel(modelId, provider) {
        if (!activeSession.value)
            return;
    ...
    function getSessionMsgs(sessionId) {
    ...
    function updateMessage(sessionId, id, update) {
        const s = sessions.value.find(s => s.id === sessionId);
        if (!s)
            return;
    ...
    async function sendMessage(content, attachments) {
        if ((!content.trim() && !(attachments && attachments.length > 0)) || isStreaming.value)
            return;
        if (!activeSession.value) {
            const session = createSession();
            switchSession(session.id);
        ...
        try {
            ...
            const schedulePersist = () => {
                if (sid !== activeSessionId.value || persistTimer)
                    return;
            ...
            const ctrl = streamRunEvents(runId, 
            // onEvent
            (evt) => {
                switch (evt.event) {
                    case 'run.started':
