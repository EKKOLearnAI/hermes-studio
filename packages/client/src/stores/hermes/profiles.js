...
export const useProfilesStore = defineStore('profiles', () => {
    ...
    const activeProfileName = ref(localStorage.getItem(ACTIVE_PROFILE_STORAGE_KEY));
    ...
    async function fetchProfileDetail(name) {
        if (detailMap.value[name])
            return detailMap.value[name];
        try {
    ...
    function clearProfileCache(profileName) {
        const prefixes = [
    ...
    async function renameProfile(name, newName) {
