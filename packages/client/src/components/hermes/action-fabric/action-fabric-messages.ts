...
const en = {
  ...
  executorBoundary: 'MCP and browser executors use profile-scoped runtimes. Action Fabric still owns target authorization and result verification.',
...
const zh: Messages = {
  ...
  executor: '执行器', executors: '执行器注册表', risk: '风险', environment: '环境', health: '健康', executorBoundary: 'MCP 与浏览器执行器使用 Profile 范围运行时；目标授权和结果验证仍由 Action Fabric 负责。', simulatorExecutor: '模拟器 · 无外部影响', internalExecutor: '内部 · Hermes 持有', connectorExecutor: '连接器 · 外部集成', mcpExecutor: 'MCP · Profile 运行时', browserExecutor: '浏览器 · 受治理会话', idempotency: '幂等性', reversibleLabel: '可逆', verification: '验证', enabled: '已启用', disabled: '已停用', roleAuthorization: '角色授权声明', allowDeclaration: '允许声明', denyDeclaration: '拒绝声明', notDeclared: '未声明', noCapabilities: '服务端未返回能力。', noExecutors: '服务端未返回执行器。', noRoles: '服务端未返回助手角色。',
