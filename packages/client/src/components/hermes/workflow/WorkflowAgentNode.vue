  ...
  <div class="workflow-agent-node" :class="[statusClass, { selected }]">
    ...
    >
      ...
      <template v-if="data.agent === 'hermes'">
        <NSelect
          :value="data.executionPolicy?.allowedToolsets"
          multiple
          tag
          filterable
          size="small"
          :disabled="data.readonly"
          :placeholder="t('workflow.node.allowedToolsets')"
          @update:value="value => updateExecutionPolicy('allowedToolsets', value as string[])"
        />
        <NSelect
          :value="data.executionPolicy?.allowedTools"
          multiple
          tag
          filterable
          size="small"
          :disabled="data.readonly"
          :placeholder="t('workflow.node.allowedTools')"
          @update:value="value => updateExecutionPolicy('allowedTools', value as string[])"
        />
        <label class="node-toggle-row">
          <span>{{ t('workflow.node.skipMemory') }}</span>
          <NSwitch
            :value="data.executionPolicy?.skipMemory === true"
            size="small"
            :disabled="data.readonly"
            @update:value="value => updateExecutionPolicy('skipMemory', value)"
          />
        </label>
        <label class="node-toggle-row">
          <span>{{ t('workflow.node.skipContextFiles') }}</span>
          <NSwitch
            :value="data.executionPolicy?.skipContextFiles === true"
            size="small"
            :disabled="data.readonly"
            @update:value="value => updateExecutionPolicy('skipContextFiles', value)"
          />
        </label>
      </template>
      <label class="node-toggle-row">
