...
function getAttachmentUrl(att: { name: string; url: string; uploadPath?: string }): string {
  const filePath = getFilePathFromAttachment(att);
  return filePath ? getDownloadUrl(filePath, att.name) : att.url;
}
        ...
        <div class="msg-content" :class="message.role">
          <div class="message-bubble" :class="{ system: isSystem }">
            <div v-if="hasAttachments" class="msg-attachments">
              ...
              >
                <template v-if="isImage(att.type) && getAttachmentUrl(att)">
                  <img
                    :src="getAttachmentUrl(att)"
                    :alt="att.name"
                    class="msg-attachment-thumb"
                    @click="previewUrl = getAttachmentUrl(att)"
                  />
                </template>
                <template v-else>
