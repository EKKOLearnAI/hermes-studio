...
async function copyLink() {
  ...
  if (ok) message.success(t('models.geminiCopyLink'))
...
<template>
  ...
  >
    <div class="gemini-login">
      <div v-if="status === 'idle' || status === 'loading'" class="gemini-login__state">
        <NSpin size="small" />
      </div>

      <div v-else-if="status === 'waiting'" class="gemini-login__state">
        <p class="gemini-login__notice">{{ t('models.geminiPolicyNotice') }}</p>
        <p class="gemini-login__hint">{{ t('models.geminiWaiting') }}</p>
      ...
      <div v-else-if="status === 'approved'" class="gemini-login__state gemini-login__state--success">
      ...
      <div v-else-if="status === 'expired'" class="gemini-login__state">
        <p class="gemini-login__error">{{ t('models.geminiExpired') }}</p>
        <NButton size="small" @click="retry">{{ t('common.retry') }}</NButton>
      </div>

      <div v-else-if="status === 'error'" class="gemini-login__state">
        <p class="gemini-login__error">{{ errorMessage }}</p>
        <NButton size="small" @click="retry">{{ t('common.retry') }}</NButton>
      </div>
    </div>
