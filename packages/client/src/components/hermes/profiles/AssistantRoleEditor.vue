...
type ContextRecipeDraft = ContextRecipeInput & { id?: string; builtIn?: boolean }
...
function reset(): void {
  ...
  recipes.value = (role?.recipes ?? []).map(({ id, name, description, builtIn, enabled, domains, sections, queryTemplate, limits }) => ({ id, name, description, builtIn, enabled, domains: [...domains], sections: [...sections], queryTemplate, limits: { ...limits } }))
...
function deleteRecipe(index: number): void { if (!recipes.value[index]?.builtIn) recipes.value.splice(index, 1) }
