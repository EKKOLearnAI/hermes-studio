...
async function onFileChange(e: Event) {
  ...
  if (file.size > 100 * 1024 * 1024) {
    error.value = t('settings.display.taFileTooLarge')
    return
  }
  ...
  try {
    ...
    if (!res.ok) {
      ...
      throw new Error(data.error || `${t('settings.display.taUploadFailed')} (${res.status})`)
    }
    await fetchStatus()
  } catch (e: any) {
    error.value = e.message || t('settings.display.taUploadFailed')
  } finally {
...
async function resetAnimation() {
  try {
    ...
    error.value = e.message || t('settings.display.taResetFailed')
  }
}
    ...
    <template v-else>
      <div class="ta-row">
        <span class="ta-status">
          {{ hasCustom ? `${t('settings.display.taCustomLoaded')} (${customType.toUpperCase()})` : t('settings.display.taDefault') }}
        </span>
        <NButton size="small" @click="openPicker" :loading="uploading">
          {{ hasCustom ? t('settings.display.taChange') : t('settings.display.taUpload') }}
        </NButton>
        <NButton v-if="hasCustom" size="small" @click="resetAnimation" :loading="uploading">
          {{ t('settings.display.taResetDefault') }}
        </NButton>
      </div>
      <div class="ta-help">
        {{ t('settings.display.taHelp') }}
      </div>
