...
async function scrollInitialPageToBottom(): Promise<void> {
  await nextTick()
  const keepBottomUntil = Date.now() + 700
  const keepBottom = () => {
    if (!scroller.value || Date.now() > keepBottomUntil) return
...
async function loadInitialHistory(): Promise<void> {
  ...
  try {
    const page = await getRoomDetail(roomId.value, { limit: PAGE_SIZE, history: true })
...
async function loadOlderHistory(): Promise<void> {
  ...
  olderError.value = ''
  try {
    const page = await getRoomDetail(roomId.value, { before, limit: PAGE_SIZE, history: true })
    ...
    if (scroller.value === element) {
      const keepAnchorUntil = Date.now() + 500
      const keepAnchor = () => {
