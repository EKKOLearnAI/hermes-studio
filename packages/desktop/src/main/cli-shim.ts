...
export function createShimContent(
  ...
  if (platform === 'win32') {
    ...
    return [
      ...
      'set "PYTHON=%RUNTIME%\\python\\venv\\python.exe"',
      'if not exist "%PYTHON%" set "PYTHON=%RUNTIME%\\python\\python.exe"',
      'set "VIRTUAL_ENV=%RUNTIME%\\python\\venv"',
      'if not exist "%VIRTUAL_ENV%\\python.exe" set "VIRTUAL_ENV=%RUNTIME%\\python"',
