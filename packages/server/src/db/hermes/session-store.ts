...
export type SessionTitleSource = 'unknown' | 'initial' | 'manual' | 'heuristic' | 'compaction' | 'llm'
...
function mapSessionRow(row: Record<string, unknown>): HermesSessionRow {
  ...
  return {
    ...
    title,
    title_source: normalizeTitleSource(row.title_source),
...
function normalizeTitleSource(value: unknown): SessionTitleSource {
  const source = typeof value === 'string' ? value.trim() : ''
  if (source === 'initial' || source === 'manual' || source === 'heuristic' || source === 'compaction' || source === 'llm') return source
  return 'unknown'
}

function titleSourceForCreate(title: string | undefined, explicit?: SessionTitleSource): SessionTitleSource {
  if (explicit) return explicit
  return title && title.trim() ? 'initial' : 'unknown'
}
...
export function createSession(data: {
  ...
  if (!isSqliteAvailable()) {
    return {
      ...
      user_id: null, model: data.model || '', provider: data.provider || '', title: data.title || null, title_source: titleSourceForCreate(data.title, data.titleSource),
  ...
  const db = getDb()!
  const titleSource = titleSourceForCreate(data.title, data.titleSource)
  db.prepare(
    `INSERT INTO ${SESSIONS_TABLE} (id, profile, source, model, provider, title, title_source, started_at, last_active, workspace)
     VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`,
  ).run(data.id, data.profile || 'default', source, data.model || '', data.provider || '', data.title || null, titleSource, now, now, data.workspace || null)
...
export function renameSession(id: string, title: string, source: SessionTitleSource = 'manual'): boolean {
  if (!isSqliteAvailable()) return false
  const db = getDb()!
  const result = db.prepare(`UPDATE ${SESSIONS_TABLE} SET title = ?, title_source = ? WHERE id = ?`).run(title, normalizeTitleSource(source), id)
  return result.changes > 0
}

export function updateAutoGeneratedSessionTitle(id: string, title: string, source: Extract<SessionTitleSource, 'heuristic' | 'compaction' | 'llm'>): boolean {
  const trimmed = title.trim()
  if (!trimmed || !isSqliteAvailable()) return false
  const db = getDb()!
  const result = db.prepare(`
    ...
    WHERE id = ?
      AND (
        title IS NULL
        OR title = ''
        OR COALESCE(title_source, 'unknown') IN ('initial', 'heuristic', 'compaction', 'llm')
      )
  `).run(trimmed, source, id)
  return result.changes > 0
}
