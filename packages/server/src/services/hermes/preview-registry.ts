...
function findInstanceIndex(state: PreviewRegistryState, previewId: string): number {
  return state.instances.findIndex(entry => entry.id === previewId)
}
...
export async function getPreviewInstance(profile: string, previewId: string): Promise<PreviewInstance | null> {
  const state = await readRegistry(profile)
  const record = state.instances.find(entry => entry.id === previewId)
