...
export const HEALTH_INTERVENTION_RULE_VERSION = 'health-interventions-v1'
...
function postureChainOverload(context: HealthRuleContext): HealthRuleCandidate | null {
  ...
  if (Array.isArray(findings)) {
    for (const finding of findings) {
      ...
      if (typeof value?.code === 'string' && (finite(value.severity) ?? 0) >= 0.5) constrained.add(value.code)
    }
  }
  if (Array.isArray(chain)) for (const item of chain) if (typeof item === 'string') constrained.add(item)
  const overlap = [...new Set(context.plan.trainingChains)].filter(item => constrained.has(item)).sort()
...
function incompleteMarkerMetadata(context: HealthRuleContext): HealthRuleCandidate | null {
  const incomplete = allMarkerSummaries(context).some(summary => typeof summary.recordId !== 'string'
...
function providerFlag(context: HealthRuleContext): HealthRuleCandidate | null {
  const flags = confirmedMarkers(context).flatMap(summary => Array.isArray(summary.markers) ? summary.markers.map(item => {
    ...
    return typeof flag === 'string' ? flag.trim().toLowerCase() : ''
  }) : []).filter(flag => PROVIDER_FLAGS.has(flag))
...
export function evaluateHealthInterventionRules(context: HealthRuleContext): HealthRuleCandidate[] {
  return [lowSleep(context), pain(context), unsafeWeightLoss(context), proteinShortage(context),
