...
/**
 * Resolve the preferred Python path for the Hermes Gateway.
 *
 * Checks HERMES_PYTHON_PATH env var first, then auto-detects the
 * user's independent hermes-agent venv. Returns null when no preferred
 * path is found, letting the CLI use its default resolution.
 */
function resolvePreferredPythonPath(): string | null {
  const envPath = process.env.HERMES_PYTHON_PATH?.trim()
  if (envPath) return envPath

  const home = homedir()
  if (process.platform === 'win32') {
    const winPath = join(home, '.hermes', 'hermes-agent', 'venv', 'Scripts', 'python.exe')
    if (existsSync(winPath)) return winPath
  } else {
    const posixPath = join(home, '.hermes', 'hermes-agent', 'venv', 'bin', 'python')
...
function startGatewayRunManagedInternal(
  ...
  const pythonPath = resolvePreferredPythonPath()
  const childEnv: Record<string, string | undefined> = {
    ...process.env,
    HERMES_HOME: profileDir,
  }
  if (pythonPath) {
    childEnv.HERMES_PYTHON_PATH = pythonPath
  }
