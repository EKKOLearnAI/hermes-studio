...
def _refresh_terminal_env() -> None:
    """Re-read terminal config from current worker HERMES_HOME/config.yaml to TERMINAL_* env vars.

    The bridge broker launches one worker process per profile. Each worker sets
    HERMES_HOME once at startup, then bridges that profile's terminal config so
    terminal_tool sees the correct backend (for example ssh instead of local).
    """
