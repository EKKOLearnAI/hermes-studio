...
def _configured_default_headers() -> dict[str, str] | None:
    """Return model.default_headers from the active Hermes profile config."""
    try:
        from hermes_cli.config import cfg_get, load_config  # type: ignore[import-not-found]

        raw = cfg_get(load_config(), "model", "default_headers")
    except Exception:
        try:
            import yaml
            ...
            if not path.exists():
                ...
                raw = model_cfg.get("default_headers") if isinstance(model_cfg, dict) else None
        except Exception:
            raw = None
...
def _merge_default_headers(kwargs: dict[str, Any]) -> dict[str, Any]:
    """Merge user-configured model.default_headers into OpenAI client kwargs."""
    headers = _configured_default_headers()
    if not headers:
        return kwargs
    merged = dict(kwargs)
    merged_headers = dict(merged.get("default_headers") or {})
    merged_headers.update(headers)
    merged["default_headers"] = merged_headers
    return merged


def _install_default_headers_patch() -> None:
    """Ensure Studio bridge OpenAI clients honor model.default_headers.

    Hermes Studio can run against a bundled Hermes Agent runtime older than the
    user's CLI checkout. Some WAF-sensitive OpenAI-compatible endpoints require
    overriding the OpenAI SDK User-Agent via model.default_headers. Patch the
    runtime client factory at the bridge boundary so both shared and per-request
    clients receive the configured headers without waiting for a runtime update.
    """
    try:
        from agent import agent_runtime_helpers  # type: ignore[import-not-found]
    except Exception:
        return
    if getattr(agent_runtime_helpers, "_hermes_studio_default_headers_patch", False):
        return
    original_create = agent_runtime_helpers.create_openai_client

    def create_openai_client_with_default_headers(
        agent: Any,
        client_kwargs: dict,
        *,
        reason: str,
        ...
        return original_create(
            agent,
            _merge_default_headers(dict(client_kwargs or {})),
            reason=reason,
            shared=shared,
        )

    agent_runtime_helpers.create_openai_client = create_openai_client_with_default_headers
    agent_runtime_helpers._hermes_studio_default_headers_patch = True
