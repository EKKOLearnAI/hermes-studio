 ...
 * 自助注册普通用户（public）。默认关闭，需环境变量
 * HERMES_WEB_UI_ALLOW_REGISTER=1 才开放。
 * 注册成功后自动登录（返回 JWT）。
 */
export async function register(ctx: Context) {
  if (process.env.HERMES_WEB_UI_ALLOW_REGISTER !== '1') {
    ctx.status = 403
    ctx.body = { error: 'Registration is disabled' }
    return
  }
  const { username, password } = ctx.request.body as { username?: string; password?: string }
  if (!username || !password) {
  ...
  if (!/^[a-zA-Z0-9_.-]{3,32}$/.test(name)) {
    ctx.status = 400
    ctx.body = { error: 'Username must be 3-32 chars, letters/digits/._-' }
    return
  }
  ...
  // 新注册用户：role=admin（非超管），绑定默认 profile（若存在）
  const user = createUser({
    username: name,
    password: String(password),
    role: 'admin',
    profiles: listProfileNamesFromDisk().includes('default') ? ['default'] : [],
    defaultProfile: listProfileNamesFromDisk().includes('default') ? 'default' : null,
  })
  if (!user) {
    ctx.status = 500
