...
import { PreviewRegistryError, getPreviewInstance, listPreviewInstances, startPreviewInstance, stopPreviewInstance } from '../../services/hermes/preview-registry'
...
export async function getPreview(ctx: any) {
  ...
  if (!previewId) {
    ctx.status = 400
    ctx.body = previewErrorBody('preview_id_missing', 'Preview id is required')
    return
  }
